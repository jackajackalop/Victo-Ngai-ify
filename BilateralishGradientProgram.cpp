...
#define BLUR_SHADER(OFFSET) \
        ...
        "       blurred_out += texelFetch(color_tex, ivec2(gl_FragCoord.xy)+OFFSET, 0)*weights[i];\n"\
        "       blurred_out += texelFetch(color_tex, ivec2(gl_FragCoord.xy)-OFFSET, 0)*weights[i];\n"\
        "   }\n"\
        ...
        "   vec4 control_in = texelFetch(basic_tex, ivec2(gl_FragCoord.xy), 0);\n"\
		...
		"   for(int i = -20; i<=20; i++){\n"\
        "       bool bleed;\n"\
        "       float ctrlxi = texelFetch(basic_tex, ivec2(gl_FragCoord.xy)+OFFSET, 0).b;\n"\
        "       if (ctrlx>0 || ctrlxi>0) {\n"\
        "           bleed = false;\n"\
        ...
        "           }\n" \
        "           if (bleed) {\n"\
        "               bleeded_out = bleeded_out+texelFetch(gradient_tex, ivec2(gl_FragCoord.xy)+OFFSET, 0)*weight41[i+20];\n"\
                "       blurred = true;\n" \
        "           } else {\n"\
        "               bleeded_out = bleeded_out+texelFetch(gradient_tex, ivec2(gl_FragCoord.xy), 0)*weight41[i+20];\n"\
        "           } \n"\
        "       } else {\n"\
        "           bleeded_out = bleeded_out+texelFetch(gradient_tex, ivec2(gl_FragCoord.xy), 0)*weight41[i+20];\n"\
        "       }\n"\
