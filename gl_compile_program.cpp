...
GLuint gl_compile_program(
	...
	return gl_compile_program({{GL_VERTEX_SHADER, vertex_shader_source},
...
GLuint gl_compile_program(
    ...
    for(auto const &pair_iter : shader_sources){
    	GLuint shader = gl_compile_shader(pair_iter.first, pair_iter.second);
